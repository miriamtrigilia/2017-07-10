...
public class Model {
	...
	public void creaGrafo() {
		...
		// Leggi la lista degli oggeti dal DB
...
//		}  oppure usando la libreria ->
	...
	private void addEdgesV1() {
		for(ArtObject aop : this.artObjects) { // di partenza
			for(ArtObject aoa : this.artObjects) { // di arrivo
				if(!aop.equals(aoa) && aop.getId()<aoa.getId() /*(1,5) e (5,1) scelgo 1,5 e butto l'altra*/) { // escludo coppie (ao,ao) per escludere i loop
	...
	//					this.graph.setEdgeWeight(e,peso); -> funziona solo con libreria graph aggiornata
	 ...
	 * Aggiungi gli archi al grafo
	 * 
	 * VERSIONE 2 - piu efficiente ** Utilizza il metodo listArtObjectAndCount per
			...
			for (ArtObjectAndCount c : connessi) {
				ArtObject dest = new ArtObject(c.getArtObjectId(), null, null, null, 0, null, null, null, null, null, 0,
						null, null, null, null, null); // l'oggetto "dest" è un ArtObject inizializzato in modo
														...
														// hashCode/equals. In questo modo può "impersonare" un
														// vertice
														// del grafo.
				// System.out.format("(%d, %d) peso %d\n", ao.getId(), dest.getId(),
				// c.getCount()) ;
	 ...
	 * Aggiungi gli archi al grafo
	 * 
	 * VERSIONE 3 - la piu efficiente di tutte ** esegue una query unica
	...
	public boolean isObjIdValid(int idObj) { // se un'azione da fare spesso devo fare una mappa.
		if(this.artObjects == null) // vuol dire che non ho ancora creato il grafo
			return false; // dovrei lanciare un'eccezione
	...
	private ArtObject trovaVertice(int idObj) {
		// trova il vertice di partenza
				...
				if(start == null) 
					throw new IllegalArgumentException("vertice "+idObj+" non esistente"); // ma è impossibile
				return start;
	...
	public List<ArtObject> camminoMassimo(int startId, int LUN) {
		...
		best = parziale; // per non avere null pointer exception.. il peso di questo è 0 
		
		// lancio ricorsione
		cerca(parziale, 1, LUN); // ho gia un elemento dentro -> livello 1
		
		return best; // restituisco il meglio che ho trovato.
	...
	private void cerca(List<ArtObject> parziale, int livello, int LUN) {
		if(livello==LUN) {
			...
			return; // non salvo nulla ma cmq NON VOGLIO PROSEGUIRE IN PROFONDITA'
			
		}
		
		//trova i vertici adiacenti all'ultimo della sequenza
		ArtObject ultimo = parziale.get(parziale.size()-1); // esiste sempre?? (almeno un elemento) -> la lista partira con il vertice start.
		
		List<ArtObject> adiacenti = Graphs.neighborListOf(this.graph, ultimo); // tutti gli adiacenti
	...
	private int peso(List<ArtObject> parziale) { // prende a uno a uno tutti gli archi della sequenza
		int peso = 0;
		for(int i=0; i<parziale.size()-1;i++) { // max valore che puo assumere i è  size-2
			DefaultWeightedEdge e = this.graph.getEdge(parziale.get(i), parziale.get(i+1)); // arco che collega i due vertici consecutivi che sto analizzando nella lista
