...
;	@FXML
	...
	void doCalcolaComponenteConnessa(ActionEvent event) {
		String idObjStr = this.txtObjectId.getText();
		
		int idObj;
		try {
			idObj = Integer.parseInt(idObjStr);
		} catch (NumberFormatException e) {
			this.txtResult.appendText("Inserire un numero intero valido\n");
			return;
		}
		
		if(!this.model.isObjIdValid(idObj)) { // controllo che ci sia almeno un obj con quell id
			this.txtResult.appendText(String.format("Non esiste alcun oggetto con id=%d\n", idObj));
			return; // senno prova comunque
		} 
		
		int dimCC = this.model.calcolaDimensioneCC(idObj);
